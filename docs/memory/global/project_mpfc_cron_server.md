...
- Scripts/files intended to be canonical (no dated copies, single source of truth) matter MORE here than in a single-environment project, because two independent unsupervised cron sessions can each "fix" the same problem differently if there's ambiguity in the skill file.
...
**Missing local browser/session profiles → false alerts (confirmed 2026-08-19):** mpfc lacks the local Chrome profiles this project depends on for some sources (Workstream login, MS Teams customer-message fetch, WhatsApp/Zalo monitor Chrome, Upwork Neural cookie extraction) — it's a headless server, no persistent logged-in browser. That morning's cron daily-report ran on mpfc and reported a "Workstream SSO outage" plus several sources as "unavailable"/session-failed, which cascaded into a false alert (TuanNT/Bailey looked like 0h — actually 8h logged under a Workstream project named "Speedventory", just invisible to mpfc's Sheets-only fallback). Running `/daily-report recheck` from the **local** machine (which has these profiles) resolved almost everything on the first try. How to apply: when a cron-produced report shows Workstream/MSTeams/WhatsApp/Zalo/Neural as unavailable or alerts derived from their absence, treat it as suspect and re-verify from local before treating it as a real finding — don't just accept mpfc's read as final for these specific sources.

**Refined 2026-08-24 — the failure mode is a Puppeteer/Chrome browser-launch failure, not just "missing profile."** A full cron `/me:daily-report` run on this host found `workstream-login.js` completing Keycloak SSO redirect ("cookies alive") 3 separate attempts in a row (no external timeout wrapper, up to 6 min each) but never capturing a token ("SSO redirected but API never fired") — and a direct Puppeteer smoke-test (`sheets-tasklog-scan.js` for non-PHP-team devs, to rule out a per-dev issue) crashed outright with `Fatal: Timed out after 30000 ms while waiting for the WS endpoint URL to appear in stdout` — i.e. the Chrome *process itself* fails to come up cleanly here, not just "no saved session." Same root cause blocked Upwork memo-check (Rory/Aysar/Neural all `login_failed`/`session_expired`) and the MS Teams Philip check (hung past 60s, killed). This is a deeper/different symptom than the 2026-08-19 "no profile" framing — worth investigating from mpfc directly (Chrome install, sandbox flags, `/tmp` vs `/var/tmp` per [[feedback_puppeteer_cron_tmpdir]]) rather than assuming the 08-19 fix (retry from local) is the only lever. **Also confirmed the correct handling held up under this failure:** the 2026-08-24 report did NOT fabricate or guess hours data — it explicitly labeled every affected section as unverifiable, left the dependent Trello items (John Yi/Bailey/Rebecca/Rory-memo/Philip) incomplete with reasons, and recommended a local recheck, rather than reporting false 0h or false-clean. Also confirmed same run: the legacy Google Sheets task-log fallback (`sheets-tasklog-scan.js`'s Sheets path) returned genuinely empty for every dev tested (5 PHP-team + 3 Fountain devs), consistent with that system being fully retired per [[reference_workstream]] — so on this host there is currently NO fallback at all for hours data, not even a stale one.
