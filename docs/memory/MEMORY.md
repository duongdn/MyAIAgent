...
- [feedback_head_truncation_missed_tuannt_bailey_hours](daily-report/sheets/feedback_head_truncation_missed_tuannt_bailey_hours.md) — 🔴 large combined-project JSON dumps: never eyeball via head, always filter/sum by employeeName across all projects before alerting 0h
...
- [feedback_upwork_access_token_needs_live_browser_touch](daily-report/upwork/feedback_upwork_access_token_needs_live_browser_touch.md) — 🔴🔴 timesheet/reports "session_expired" ≠ real login needed; open upwork.com once in carrick's real Chrome to refresh access token, ~10s fix
...
- [feedback_report_path_is_root_reports_not_plans_reports](server-monitor/feedback_report_path_is_root_reports_not_plans_reports.md) — 🔴🔴 recurring: reports go to root `reports/`, NOT `plans/reports/`
