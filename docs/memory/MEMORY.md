...
- [feedback_workstream_fetch_needs_explicit_date_arg](daily-report/sheets/feedback_workstream_fetch_needs_explicit_date_arg.md) — workstream-fetch-project-week.js w/o date arg silently defaults to current (empty) week; always pass PREV_DATE explicitly
- [feedback_workstream_vs_sheets_migration_gaps](daily-report/sheets/feedback_workstream_vs_sheets_migration_gaps.md) — script missing 8 WS projects (fixed); new untracked "Crystal lang"/Arthur project found (PhucVT dev); TuanNT sheet-only for Charles Chang despite WS membership; Speedventory has no dev registered in WS
...
- [feedback_check_workstream_before_flagging_shortfall](daily-report/sheets/feedback_check_workstream_before_flagging_shortfall.md), [feedback_marginal_daily_shortfall_check_weekly](daily-report/sheets/feedback_marginal_daily_shortfall_check_weekly.md) — 🔴5x recurred: NEVER trust one WS query for a 0h/shortfall claim, re-run + take higher; LeNH+Peptide Clyde chronic offender; marginal shortfall (<1h) check weekly total first
...
- [feedback_warning_needs_explanation](bailey/feedback_warning_needs_explanation.md), [reference_bailey_monitor_skill_file](bailey/reference_bailey_monitor_skill_file.md) — Slack #maintenance WARNING/NOT OK labels must always carry inline plain-language explanation, never a bare status word
