...
**Why some scripts worked, one didn't:** `upwork-neural-check.js`, `upwork-memo-check.js`, `upwork-room-messages.js` have a **fallback to system `python3`** (miniconda 3.13, matching lz4/browser_cookie3). `upwork-weekly-hours.js` didn't — fixed 2026-08-07 by adding the same venv→system fallback loop.

**Second root cause, fixed 2026-08-10:** the shared cookie extractor `scripts/get-carrick-upwork-cookies.py` ITSELF forced the broken venv's site-packages into ANY interpreter regardless of which python called it, so even scripts with a fallback loop still failed. Fixed to prefer the interpreter's own working `browser_cookie3`, injecting the venv's site-packages only as a last resort (`try: import browser_cookie3 / except ImportError: <inject venv path>`). This was the real cause of an earlier "manual re-auth needed" appearance — never an auth issue.
