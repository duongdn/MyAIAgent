...
**Excluded from default full run (2026-08-19):** Per-chat full CDP message extraction (open each chat, scroll, parse DOM) burns significant tokens across both pieces every run. User asked to drop them from the default `/daily-report` full run and gate behind a flag. `.claude/commands/me/daily-report.md` now: (1) skips Pieces 16-17 in both cron sequential and interactive parallel full-run flows unless `--include-whatsapp-zalo` is passed, (2) keeps standalone `/daily-report whatsapp` / `/daily-report zalo` always available regardless of the flag.
