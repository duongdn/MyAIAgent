...
Piece 14 is informational-only (no Trello gate yet). First live test 2026-07-08 found OhCleo has 2 slow endpoints (MediaRecommendsView.get ~19.8s, HomeMediasView.get ~17.8s, both >>5s threshold) — worth surfacing to user, not yet actioned. MPFC apdex 0.57 (poor, <0.7 threshold) — also worth surfacing.

**🔴 CONFIRMED 2026-07-09 — now standard, not on-demand:** user noticed it missing from the daily report ("Where performance report ??? the new one I added yesterday") and confirmed it should run every time going forward — added to both the cron pipeline and the interactive parallel-launch step in `.claude/commands/me/daily-report.md`. Also: a summary-only table is NOT enough — user asked for "chi tiết" (detail) after seeing just the summary row once. Every run must append the FULL `topErrors` list (every facet+count) and FULL `slowestTransactions` list (every endpoint+avgMs+calls), not a truncated "if any" mention. Found on this run: MPFC's slow transactions are an active SQL injection scanner (`WAITFOR DELAY` payloads on `/search/`) plus a broken JSON API plugin method — both real, actionable issues, not monitoring noise.
