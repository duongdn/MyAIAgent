...
- "Elliott" because KhanhHH had Fri 7.83h vs 8h target (0.17h shortfall). But per `feedback_lenh_consolidated` ("any combined shortfall = real alert"), "the Trello checklist completion can stay (don't un-complete for tiny gaps), but the report must mark Status as ⚠️ and include the reminder line." Reminder was sent → Trello should have been completed.
...
4. Pair with `feedback_project_topics_not_alerts` and `feedback_low_activity_devs_not_alert` (Neural Contract entry) — same principle: the per-client item gates on the named owner's reachability/comms, not on every operational signal in the project's blast radius.
5. Tiny daily shortfalls (<1h) that already trigger a Matrix reminder do NOT block Trello (per `feedback_lenh_consolidated` ("any combined shortfall = real alert") last sentence — "checklist completion can stay").
