...
**Re-verified next morning (2026-07-14, ~19h after issuance):** `whoami` still 200 OK, unchanged token, no refresh needed overnight. A detached background poller (nohup'd, meant to log checkpoints at 30m/1h/3h/6h/12h) died after the 3h mark when the host/session restarted — so the 6h/12h checkpoints were never captured that way — but this direct next-day manual check is actually stronger evidence than the missed automated ones. At this point treat the token as durably long-lived; stop re-verifying defensively on every report and only investigate if a real `whoami` failure occurs.
