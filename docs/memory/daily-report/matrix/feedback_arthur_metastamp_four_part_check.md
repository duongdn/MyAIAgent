...
## 🔴 CRITICAL bug found 2026-07-07 — `arthur_monitor.last_run` must equal the actual data-read cutoff, NOT feature-build/run-completion time

When first building Piece 13, `config/.monitoring-timelines.json`'s `arthur_monitor.last_run` was set to `14:29` — the time the incremental-check feature finished being built/run — instead of `~10:27`, the time through which the ORIGINAL full-history report had actually verified data. This silently created an unmonitored ~4-hour gap (10:27-14:29). That exact gap contained the single most important update in the whole engagement: all 7 client-requested bugs + the attribution/metadata fix, confirmed deployed to production by Nick at 15:06. The first incremental run ("nothing new, 20-min window") was wrong and had to be fully rewritten after the gap was discovered by accident (user asked about "bug #7", broader search stumbled onto the missed messages).

**Rule going forward, applies to ANY incremental/timeline-based monitoring piece, not just Arthur:** `last_run` must be set to the timestamp through which data was actually read and verified — normally the timestamp of the LAST message/event actually included in the report — never the wall-clock time the command/session happened to finish. If a report's data cutoff and the command's finish time differ (e.g. research/writing took a while after the last fetch), always use the data cutoff.

**Fix applied 2026-07-07:** `arthur_monitor.last_run` corrected to `2026-07-07T15:46:00+07:00` (true cutoff of the corrected 2nd-pass report), with an explicit warning about this added to the `description` field in the timelines JSON itself so it survives even if this memory file isn't read.
