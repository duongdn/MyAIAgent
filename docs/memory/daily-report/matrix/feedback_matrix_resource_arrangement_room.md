...
**Recurred 2026-08-11 (TuanNT) — NEW nuance: leave notes can post AFTER the report's Matrix snapshot.** Same shape as 06-18: TuanNT 0h on 08-10 written as "not logged yet, likely late logging", John Yi/Bailey/Rebecca gates left open. Real cause: namtv posted `TuanNT 10/08 - 11/08/2026 Về quê => Chờ a Năm update plan ==> Bên Bailey ko bù` at **08:59 on 08-11** — AFTER the 07:42 cron fetch AND the 08:55 recheck fetch (both fetched the 07:16 transcript). So even a correctly-done cross-check misses it; the note lands after the data window. **Recovery:** before sending reminders / finalizing "late logging" framing for ANY 0h dev, do a fresh Matrix fetch and grep the Resource Arrangement room for that exact date — namtv can post leave minutes-to-hours after the report is drafted. If found, reframe 0h as confirmed leave, complete the gated Trello items, update timelines. The "likely late logging" label is provisional until the fresh fetch confirms it — the leave check is a REQUIRED step, not a nice-to-have, whenever a 0h/shortfall is about to be framed or a gate held.
