...
**Related exception confirmed 2026-07-09:** `long an res` (also walletType 3 in the API) is likewise mis-suggested as Investment by raw walletType — it should be grouped under 🏠 **Real Estate** with "Nhà" instead. Verified by checking every `reports/money-history.json` snapshot: all use `real_estate = Nhà + long an res = 3,520,000,000` consistently, and the money-report skill's own category table lists "long an res" as its real-estate example. Before finalizing any new run's categorization, cross-check `money-history.json`'s `categories` schema — it's the ground truth for what's been consistently applied.
