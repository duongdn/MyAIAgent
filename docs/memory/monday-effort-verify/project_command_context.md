...
- Elena Klebanov (SamGuard) and Celine Fierro (OhCleo) are Workstream projects NOT in `reference_workstream.md`'s 17-project list and not returned by self `/time/projects` either — a similar blind spot to LegalAtoms. `/admin/projects` = 403, no listing endpoint found to discover their IDs directly.
- **Update 2026-07-13:** Celine Fierro/OhCleo blind spot self-resolved — the project now appears in the live `/time/projects` self-membership list (grew to 20 projects), verified matching 3.50h. Elena Klebanov stayed unverifiable for 3 straight weeks (06-22 → 07-13) until the user supplied a separate Google Sheet task log for that project — see [[reference_elena_samguard_tasklog_sheet]]. Use that sheet for Elena Klebanov going forward instead of Workstream.
- **Update 2026-08-10 (week 08-03→08-09):** Live `/time/projects` self-membership list now returns **22 projects**, including 3 NOT in `scripts/workstream-fetch-project-week.js`'s static map: **Auction Warehouse** (`cms5wsucz149dlm1vrf92bshf`, PhucVT 9.5h), **Samguard** (`cmqyvio6u0021qo0xljhu70q8` — the Elena Klebanov project ID, resolving the long-standing unresolved question; members TriNM/TuanNTG/KietNHT/DongNV/TienND2, NOT DuongDN), and **Others** (`cmpqae4pd0006qa1wo85fzvji`, manager-view HTTP 403 but self-view works). Full self-view sweep of all 22 confirmed DuongDN's only hours that week = Marcel/Tokenlite 8.50h. Consider adding these 3 to the static PROJECTS map (auction_warehouse, samguard, others).
...
**Run 2026-08-17 (week 2026-08-10 to 08-16):** clean run, all 3 lines in ThuyLTT's image matched independent sources exactly — Speedventory/Bailey Joey 1.00h (Paturevision sheet W40, "Weekly Monitor Aug 2026"), Tokenlite/Marcel 0.50h (Workstream self-view), Samguard/Elena 0.00h (Workstream self-view AND [[reference_elena_samguard_tasklog_sheet]] agree). Full sweep of all 22 live-accessible Workstream projects found no project with >0h absent from the image. Workstream access_token had expired (`"exp" claim timestamp check failed`) — re-ran `workstream-login.js`, verified via direct `/me` call (`fullName: "DuongDN"`) rather than trusting the script's own "Verified" log line (which printed `fullName: undefined` despite success).
