...
- Columns: all audited annual years (`type=HK`) + last `max_quarters` unaudited quarters (`type=H`), left→right
- Section order: Tài sản → Nguồn vốn → Kết quả kinh doanh → Lưu chuyển tiền tệ (HĐKD/HĐĐT/HĐTC)
...
- EPS rows (code 70/71, "Đồng/1 cổ phiếu") are NOT divided by 1e9 — kept as raw đồng, unlike all other rows which are tỷ đồng
- Balance check (270 vs 440) runs on annual data only, tolerance 10M VND
- No restriction on ticker type — banks/securities dumped as-is, whatever cafef returns
