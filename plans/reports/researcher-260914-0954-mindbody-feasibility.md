...
- Đọc thẳng code thật (`barcode_ajax.php` trên server, xem chi tiết ở mục C) — **để chính xác: code CÓ đọc kết quả trả về** (`if ($response->ArrivalAdded == 1)`), tức là có check cuộc gọi `addarrival` tới Mindbody thành công hay không. Cái THIẾU cụ thể là **không có bước gọi thêm `GetClientServices` để kiểm tra member đó có entitlement/quyền hợp lệ hay không** trước khi quyết định mở cửa — nghĩa là code chỉ biết "Mindbody đã ghi nhận có người tới", không biết "người đó có được phép vào hay không". Ngay cả BXR cũng chưa giải xong đúng phần "kiểm tra đúng quyền trước khi cho phép" mà Wildsoul đang cần ở mục A lẫn C.
...
**Kết luận mục A:** Khả thi 1 phần — **data thô Mindbody có sẵn** (`GetClientServices`, `GetClasses`), nhưng **không có sẵn endpoint/logic tự giải thích lý do fail** — phần đó NUS phải tự ghép lại thành rule engine từ data thô đó. Có bằng chứng thật (bug `$rem=1`, bug `Error.Code` trùng ở web BXR; waiver phân mảnh giữa web/mobile) cho thấy chính NUS cũng từng làm sai đúng kiểu lỗi này khi tự ghép logic. Nên áp dụng thiết kế 2 tầng + decision-trace của ChatGPT, đo độ chính xác checker bằng cách so với kết quả booking thật trong sandbox trước khi giao cho Wildsoul dùng thật, và bắt buộc có test coverage cho cả case fail lẫn pass (rút kinh nghiệm từ bug BXR).
...
- **⚠️ Lỗ hổng thật, chính là gốc của ticket BXR-88 nêu ở mục A — nói chính xác lại:** code **CÓ đọc kết quả trả về từ Mindbody** (`if ($response->ArrivalAdded == 1) { $added = 1; }` → xanh "WELCOME", ngược lại → đỏ "DENIED") — đây là logic kiểm tra thật, không phải "không kiểm tra gì". Cái thiếu cụ thể là: `addarrival` chỉ trả lời "Mindbody có ghi nhận lượt check-in này không" (thường luôn thành công nếu ClientId/LocationId hợp lệ), **KHÔNG trả lời "member này có đang có membership/entitlement hợp lệ để vào cửa hay không"** — vì code không gọi thêm `GetClientServices` để hỏi câu đó. Nói cách khác: có bước đọc kết quả booking/check-in, nhưng thiếu bước đọc quyền/entitlement — 2 việc khác nhau, và việc thứ 2 mới là cái Wildsoul cần ("validate entitlement, không cho vượt hạn mức"). Đây là phần **BXR chưa từng làm** — với Wildsoul, bước check entitlement trước khi cho vào/cho book là phần việc THẬT SỰ MỚI, không tái dùng nguyên code cũ được, cần ước lượng effort riêng, đừng để khách nghĩ đây là "đã có sẵn, chỉ cần sao chép".
