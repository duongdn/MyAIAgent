...
**Timeline:** Uses `daily_report.last_run` from `config/.monitoring-timelines.json` as window start — this applies to EVERY piece (Slack, Discord, Matrix, Sheets, email, etc.), not just some of them. `last_run` is authoritative as long as it's being updated correctly (see timeline update rules below) — do NOT hardcode a day-of-week fallback (e.g. "if today is Monday, start from Friday") as the primary mechanism. Only fall back to a calculated previous-business-day window if `last_run` is provably missing, corrupted, or clearly stale (e.g. >72h old with no explanation) — and even then, that's a sanity-check fallback, not a substitute for fixing why `last_run` went stale in the first place.
...
🔴 **Full run (no piece arg) is INLINE-BY-DEFAULT, `--cron` or not.** User confirmed 2026-08-20: full daily-report runs daily, individual pieces rarely used — so per-piece subagent spawning is pure overhead every day, not an occasional cost. Execute ALL pieces sequentially in THIS session (same rules as `--cron` above: no subagent/Task spawns per piece). Each spawned subagent would reload this entire 1250+ line command file plus all matched memory from scratch — that reload is the dominant token cost of a full run. Only spawn a subagent for a piece when the user explicitly runs that piece standalone (e.g. `/daily-report elena` alone) AND the work is long-running/needs isolation — not as the default full-run pattern.
...
|-----------|------|-----------|-----------|-------------|
| Baamboozle | baamboozle | xoxp | General activity. **Aysar gate is the MPDM channel `C07SQ4HAUHZ` specifically** (Carrick's "Today's update" — NO fixed time, seen 08:41-09:03 AND 17:00-17:45; never assume "not due yet") — general workspace noise is NOT the gate. 🔴🔴🔴 **MPDM silence is ONLY an alert if KhanhHH has Workstream hours on Baamboozle for that day. BEFORE writing anything about MPDM silence, check Workstream baamboozle project hours first (`node scripts/workstream-fetch-project-week.js <date> baamboozle`). 0h logged = no task = silence is NORMAL, not an alert — do not report it, do not list it, do not leave Aysar incomplete over it.** This exact mistake has recurred 5+ times (see `feedback_aysar_consolidated.md`) — this line exists specifically because linking to the memory file was not enough to stop it. | Aysar |
| RDC - FM Monitoring | rdc | xoxp | dmetiner updates | Franc |
| Swift Studio | swift | xoxp | Carrick activity | Rory |
| Xtreme Soft Solutions | xtreme | xoxp | 🔴 **Kai daily report — CONDITIONAL on Workstream, and on ROLE:** LongVV plays 2 roles on this project — Kai's role (needs daily report) and Brian's role (WordPress work, no report needed). First check Workstream project `maddy` (`cmpqc1v7v00ahtk1vs1817xt8`) for LongVV's hours on the reporting date. If 0h that day, skip the report-presence check entirely (not an alert). If >0h, inspect the task TEXT: if ALL that day's hours are WordPress-tagged (Brian's role), skip the check too — no report expected. Only if any hours are Kai-role (non-WordPress) tasks, THEN search Slack for "progress"/"daily report" — missing report on a day he did Kai-role work IS an alert (see [[feedback_kai_daily_report_gate]]). Always still check for unanswered Madhuraka/client messages + blockers regardless of hours. | Maddy - Carrick/Kai/Luis |
...
| — (see Matrix, not Slack) | — | — | 🔴 **Bailey ALSO gated on Nick's daily report to the customer in Matrix room "NUS - Bailey - Paturevision 2026" (`!MaisjkNOhxoXkhCxqa:nustechnology.com`) — added 2026-09-08, same pattern as Aysar's MPDM gate.** Check the room's message history since `daily_report.last_run` for a daily-report-shaped message from Nick (task summary to the customer, not just dev chat). If missing for a day Nick had Workstream/Sheets hours on Bailey/Paturevision, this is an alert — flag it, but do NOT auto-send a reminder into the room; surface it in the report and ask the user for explicit confirmation before sending (same confirm-first rule as every other reminder in this skill — see Piece 9's `--send-reminder` gate). Real incident that prompted this: 2026-09-08, datnc noticed no daily report from Nick since start of Sept, duongdn had to remind the team live in-room that the daily-report habit is mandatory ("bảo hiểm communication khi mình làm việc với cus"). | Bailey |
...
| `bailey` | Work | Bailey | `slack ggs` + `sheets tuannt` (TuanNT 0h-across-5-sheets also gates this; VietPH resigned 2026-06-30, no longer a source) + **Matrix "NUS - Bailey - Paturevision 2026" (`!MaisjkNOhxoXkhCxqa:nustechnology.com`) — Nick's daily report to customer, added 2026-09-08.** Bailey Upwork workrooms (DEV1/DEV3, accounts vinn/david2) REMOVED 2026-08-10 — no longer memo-checked. |
...
**CONFIRMED 2026-07-13 by user: now part of every Full Run** (previously on-demand-only). Gates Trello item "Arthur - Meta-Stamp" (Work checklist, "Check progress" card) — complete it if no new unresolved issues from this run. Also runnable standalone via `/daily-report arthur` or `/me:arthur-monitor` alias. Language: **Vietnamese always** — user cannot read English well and most raw client messages are in English.
