// Daily email scan for 2026-05-26 — window: 2026-05-25T09:05+07 → now
const tls = require("tls");
const accounts = require("../config/.email-accounts.json").accounts;

const WINDOW_START = new Date("2026-05-25T09:05:00+07:00");
const IMAP_SINCE = "25-May-2026"; // search server-side; filter by Date header >= WINDOW_START
    ...
    socket.on("data", (data) => {
      ...
      if (step === 0 && buffer.includes("OK")) {
        ...
        for (const block of blocks) {
          ...
          subjects.push({ subject: sub.trim().slice(0, 80), from: from.trim().slice(0, 60), isAlert });
          if (isAlert) alerts.push(sub.trim().slice(0, 80));
        }
