// CORRECTED Daily email scan for 2026-06-02 — window: 2026-06-01T09:00+07 → now
const tls = require("tls");
const accounts = require("../config/.email-accounts.json").accounts;

// Correct window: previous daily report ran Mon Jun 1 at 09:00 +07
const WINDOW_START = new Date("2026-06-01T09:00:00+07:00");
const IMAP_SINCE = "01-Jun-2026";
...
const ACCOUNT_FILTERS = {
  "carrick@nustechnology.com": ["Generator", "Elliott", "Redmine"],
  "nick@nustechnology.com": ["John Yi"],
  "rick@nustechnology.com": ["Rollbar", "BugSnag", "Fountain", "InfinityRose", "production"],
  "kai@nustechnology.com": ["Jira", "Madhuraka"],
  "ken@nustechnology.com": ["Precognize"],
};
  ...
  return new Promise((resolve) => {
    ...
    socket.on("data", (data) => {
      ...
      if (step === 0 && buffer.includes("OK")) {
        ...
        const extraFilters = ACCOUNT_FILTERS[acct.email] || [];
        for (const blk of blocks) {
          ...
          const isAlert = ALERT_KEYWORDS.some(k => sl.includes(k));
          const isFiltered = extraFilters.length === 0 || extraFilters.some(f => sl.includes(f.toLowerCase()));
          if (isAlert && (extraFilters.length === 0 || isFiltered)) alerts.push(subj);
        }
