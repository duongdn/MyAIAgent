#!/usr/bin/env python3
"""Weekly Fountain scan for W30 (June 9-13, 2026).
...
# ── Part 2: Find W30 tab via Summary ──────────────────────────────────────────
print("Finding W30 tab...", file=sys.stderr)
...
for row in summary_rows:
    ...
    if start_d and end_d and start_d <= TARGET_MONDAY <= end_d:
        fountain_tab = tab
        print(f"  W30 = {tab} ({start_d}→{end_d})", file=sys.stderr)
        break

if not fountain_tab:
    print("  ERROR: Could not find W30 tab", file=sys.stderr)
...
dev_capacity_wk = 88  # h/wk (from W30 plan: VuTQ40+ViTHT40+ThinhT? — will use 88 based on plan)
