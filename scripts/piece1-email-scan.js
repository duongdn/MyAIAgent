...
// Window
const windowStart = new Date("2026-04-21T08:27:00+07:00");
const windowEnd   = new Date("2026-04-23T08:55:40+07:00");

function decodeMime(str) {
  if (!str) return "";
  // decode =?UTF-8?B?...?= / =?UTF-8?Q?...?=
  ...
  return new Promise((resolve) => {
    ...
    socket.on("data", (data) => {
      ...
      if (step === 2 && /A2 (OK|NO|BAD)/i.test(buffer)) {
        ...
        step = 3; buffer = "";
        send("A3", "SEARCH SINCE 20-Apr-2026");
        return;
      }
      ...
      if (step === 4 && /A4_\d+ (OK|NO|BAD)/i.test(buffer)) {
        ...
        // each response starts with "* N FETCH"
        const respBlocks = buffer.split(/(?=^\* \d+ FETCH)/m);
        for (const b of respBlocks) {
          if (!/^\* \d+ FETCH/m.test(b)) continue;
          // find the header block: everything between {NNN} and the closing )
          // Look for BODY[HEADER...] {size}
