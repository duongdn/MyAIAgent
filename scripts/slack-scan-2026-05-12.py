#!/usr/bin/env python3
"""Slack scan for 2026-05-12 daily report.

Window: 2026-05-11 08:42:58 +07 -> 2026-05-12 08:58:00 +07.
Skip SoCal Auto Wraps (dropped 2026-05-11).
"""
import json
import os
import time
...
CUTOFF_EPOCH_START = 1778463778  # 2026-05-11 08:42:58 +07
CUTOFF_EPOCH_END = 1778551080    # 2026-05-12 08:58:00 +07
QUERY = "after:2026-05-10"

SKIP_WORKSPACES = {"SoCal Auto Wraps"}
...
def load_workspaces():
    ...
    for acct in data.get("accounts", []):
        if acct["workspace"] in SKIP_WORKSPACES:
            continue
        ws = {
            "name": acct["workspace"],
            "token": acct["token"],
...
def slack_search(workspace):
    ...
    while page <= total_pages and page <= max_pages:
        ...
        for m in matches:
            ts = float(m.get("ts", 0))
            if CUTOFF_EPOCH_START <= ts <= CUTOFF_EPOCH_END:
