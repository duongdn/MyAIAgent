"""Weekly hours fetch for week of 2026-05-25 to 2026-05-29 (W28).
...
TARGET_DATES = ['25/05', '26/05', '27/05', '28/05', '29/05']
...
SHEETS = {
    ...
    'paturevision':     ('1dpFpn8-1AGAcaKczHHoVr1OaIxDQkmUNiN93sa2XBkg', ['VietPH', 'TuanNT', 'VuTQ']),
    'john_yi':          ('1xwimT6AFGfAGpVHlDA2PYxKX405Nu77dNExWBmbnytQ', ['TuanNT']),
    'william_bills':    ('1wrsg-lAWDnCEFUNk4YUTcqThMN6hy7GnXWOEW_e8NJ4', ['TuanNT']),
    ...
    'aysar_baamboozle': ('1DCsXm5SJdIep4qjr_J_tUJPasHxPEc-tzN2q2SGsOq8', ['LeNH', 'KhanhHH']),
    'fountain':         ('1iIKfjAh857qzrR2xkUWPcN_9bFAwB1pL8aJWTRk4f4o', ['VuTQ', 'ThinhT', 'ViTHT', 'PhatDLT', 'HungPN', 'HaVS', 'DatNT', 'LamLQ']),
...
def find_week_tab(sid, label):
    try:
        r = svc.values().get(spreadsheetId=sid, range='Summary!A4:BZ300').execute()
    ...
    for i, row in enumerate(rows):
        ...
        if any(p in b for p in TARGET_MONDAY_PATTERNS):
            ...
            break
    if not week:
        # Print nearby rows to diagnose
        print(f'  [{label}] Could not find May 25. Recent Summary rows:')
        for row in rows[-10:]:
            if len(row) >= 2:
                print(f'    {row[0]} | {row[1]}')
...
def parse_w_tab(sid, week_label, employees):
    try:
        r = svc.values().get(spreadsheetId=sid, range=f'{week_label}!A1:O500').execute()
    except Exception as e:
...
print('=== Weekly Hours Fetch — W28 of 2026-05-25 ===\n')

results = {}
for label, (sid, employees) in SHEETS.items():
    week, start, end, rows, err = find_week_tab(sid, label)
    if err:
        print(f'[{label}] ERROR: {err}\n')
        continue
    if not week:
        print(f'[{label}] Week starting May 25 NOT found in Summary\n')
        continue
...
for emp, projs in results.items():
    ...
    for proj_name, p in projs.items():
        st = p['summary_total']
        print(f'  - {proj_name:20s} ({p["week"]}): {st}h')

with open('/tmp/weekly-2026-05-25.json', 'w') as fh:
    json.dump(results, fh, indent=2, default=str)
print('\nWrote /tmp/weekly-2026-05-25.json')
