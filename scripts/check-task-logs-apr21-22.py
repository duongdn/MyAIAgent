...
=======
"""Piece 4: Tue 21/04 + Wed 22/04 task log hours per dev.

Both days are in the current W-sheet (Mon 20 was start of new week).
>>>>>>> 4127a9e (auto: 2026-04-23 09:00)
"""
import time, json
...
=======
# (name, sheet_id, dev_name, W-sheet for current week)
checks = [
    ("maddy",         "1PHW76CuJ7nEJ3bU150iIVFsVXrQOmP5lVKgfI4ESR7I", "LongVV",  "W3"),
    ("james_diamond", "1XUJ7Ww8dyxv6L42wtQ_7jz4GCGvBzDUXEc7YTHrKgeI", "PhucVT",  "W22"),
    ("generator",     "1LVj66VKCe8ShqR9YNAet-d3EgEBIUWaY0ooYSdHkeEM", "KhanhHH", "W37"),
    ("paturevision",  "1dpFpn8-1AGAcaKczHHoVr1OaIxDQkmUNiN93sa2XBkg", "VietPH",  "W24"),
    ("paturevision_tuannt", "1dpFpn8-1AGAcaKczHHoVr1OaIxDQkmUNiN93sa2XBkg", "TuanNT", "W24"),
    ("rory",          "1jKz9td9NgC_Iebmr3juD5Usi_7iBTu6psXI7eEuZCm8", "LeNH",    "W8"),
    ("franc",         "1RqY8DUQg0OD8wlufOO77Lg7cQ44DyonoArNHSyZztaQ", "LeNH",    "W21"),
    ("aysar",         "1DCsXm5SJdIep4qjr_J_tUJPasHxPEc-tzN2q2SGsOq8", "LeNH",    "W21"),
    ("john_yi",       "1xwimT6AFGfAGpVHlDA2PYxKX405Nu77dNExWBmbnytQ", "TuanNT",  "W20"),
    ("rebecca_tuan",  "1wrsg-lAWDnCEFUNk4YUTcqThMN6hy7GnXWOEW_e8NJ4", "TuanNT",  "W21"),
    ("rebecca_lenh",  "1wrsg-lAWDnCEFUNk4YUTcqThMN6hy7GnXWOEW_e8NJ4", "LeNH",    "W21"),
    ("rebecca_long",  "1wrsg-lAWDnCEFUNk4YUTcqThMN6hy7GnXWOEW_e8NJ4", "LongVV",  "W21"),
>>>>>>> 4127a9e (auto: 2026-04-23 09:00)
]
...
print(f"=== Task log check — Tue 21/04 + Wed 22/04/2026 ===\n")

results = []
for name, sid, dev, ws_cur in checks:
    rng_suffix = "!A1:K500"
    try:
        res = sheets_api.values().get(spreadsheetId=sid, range=f"'{ws_cur}'{rng_suffix}").execute()
        rows = res.get('values', [])
        tue_h, tue_leave, tue_nrows = day_hours(rows, 'Tue', TUE_TOK, dev)
        wed_h, wed_leave, wed_nrows = day_hours(rows, 'Wed', WED_TOK, dev)
        time.sleep(0.6)

        results.append({
            'name': name, 'dev': dev, 'ws_cur': ws_cur,
            'tue': tue_h, 'tue_leave': tue_leave, 'tue_rows': tue_nrows,
            'wed': wed_h, 'wed_leave': wed_leave, 'wed_rows': wed_nrows,
        })
        def status(h, leave):
            ...
            if h > 0: return "VLOW"
            return "ZERO"
        print(f"{name:22s} | {dev:8s} | Tue({ws_cur}): {tue_h:5.2f}h {status(tue_h, tue_leave):8s} leave={tue_leave or '-':12s} | Wed({ws_cur}): {wed_h:5.2f}h {status(wed_h, wed_leave):8s} leave={wed_leave or '-'}")
    except Exception as e:
        print(f"{name:22s} | {dev:8s} | ERROR: {str(e)[:180]}")
...
for r in results:
    ...
    key = r['dev']
    devs.setdefault(key, {'tue': 0.0, 'wed': 0.0,
                         'tue_leave_any': False, 'wed_leave_any': False, 'sheets': []})
    devs[key]['tue'] += r['tue']
    devs[key]['wed'] += r['wed']
    if r['tue_leave'] == 'full_day_off':
        devs[key]['tue_leave_any'] = True
    if r['wed_leave'] == 'full_day_off':
        devs[key]['wed_leave_any'] = True
    devs[key]['sheets'].append(f"{r['name']}(Tue:{r['tue']:.1f}/Wed:{r['wed']:.1f})")

for dev, d in sorted(devs.items()):
    print(f"{dev:8s} | Tue={d['tue']:5.2f}h tue_leave={d['tue_leave_any']} | Wed={d['wed']:5.2f}h wed_leave={d['wed_leave_any']} | {' '.join(d['sheets'])}")

with open('/tmp/piece4_tasklog_21_22.json', 'w') as f:
    json.dump({'per_sheet': results, 'per_dev': devs}, f, indent=2, default=str)
print("\nJSON saved to /tmp/piece4_tasklog_21_22.json")
>>>>>>> 4127a9e (auto: 2026-04-23 09:00)
