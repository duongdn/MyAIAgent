// Daily Discord scan for 2026-06-01 (Monday) — window: 2026-05-29T08:21+07 → now
...
const WINDOW_EPOCH_MS = new Date("2026-05-29T08:21:00+07:00").getTime();
...
async function scanAccount(acct) {
  ...
  // Verify token via /users/@me only (avoid double guilds call → 429)
...
(async () => {
  ...
  for (const acct of accounts) {
    ...
    // Only scan nusvinn (AirAgri) and nuscarrick (Bizurk). Skip HOMIEAPP accounts.
    if (acctId.toLowerCase().includes("homie")) {
      continue;
    }
    if (!first) await sleep(2000); // avoid rate limiting between accounts
    first = false;
