#!/usr/bin/env python3
"""Piece 2 scan for 2026-04-20 daily report.
Window: 2026-04-17 08:50 +07 -> now.
"""
import json
import os
import time
...
CUTOFF_EPOCH = 1776390600  # 2026-04-17 08:50:00 +07:00
QUERY = "after:2026-04-16"  # slack `after:` excludes named date, so use day before
...
def summarize(result):
    ...
    for ch_name, ch_msgs in sorted(channels.items(), key=lambda x: -len(x[1])):
        ...
        if len(ch_msgs) > 5:
            lines.append(f"  - ... and {len(ch_msgs)-5} more")
    return "\n".join(lines) + "\n"


if __name__ == "__main__":
    print(f"# Slack Piece 2 Scan — {time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    print(f"Cutoff: 2026-04-17 08:50:00 +07 (epoch {CUTOFF_EPOCH})")
    print(f"Query: {QUERY}")
    print()
