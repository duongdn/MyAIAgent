#!/usr/bin/env node
/**
 * Email refresh — 2026-05-05 09:30 → 15:30 +07:00 window
 * Scans 6 Zoho IMAP accounts. Uses BODY.PEEK[HEADER] for Subject/Date/From.
 * Window filtered in JS by Date header to avoid IMAP UTC vs +07 gotchas.
 */
const tls = require("tls");
const accounts = require("../config/.email-accounts.json").accounts;

const windowStart = new Date("2026-05-05T09:30:00+07:00");
const windowEnd   = new Date("2026-05-05T15:30:00+07:00");
...
function scan(acct) {
  return new Promise((resolve) => {
    let buffer = "";
    let step = 0;
    const results = { email: acct.email, count: 0, messages: [], error: null };
    let ids = [];
    let fetchIndex = 0;
    ...
    function send(tag, cmd) { socket.write(tag + " " + cmd + "\r\n"); }
    ...
    socket.on("data", (data) => {
      ...
      if (step === 2 && /A2 (OK|NO|BAD)/i.test(buffer)) {
        if (!/A2 OK/i.test(buffer)) {
          results.error = "select_fail: " + buffer.trim().slice(0,120);
          try{socket.destroy();}catch(e){}; resolve(results); return;
        }
        step = 3; buffer = "";
        // SINCE 4-May UTC to safely cover 5-May window in +07
        send("A3", "SEARCH SINCE 4-May-2026");
        return;
      }
      ...
      if (step === 4 && /A4_\d+ (OK|NO|BAD)/i.test(buffer)) {
        ...
        buffer = "";
        if (fetchIndex < ids.length) fetchNext();
        else {
