#!/usr/bin/env python3
"""Task log check for Thursday 23/04/2026.
Same W-week as Mon 20 / Tue 21 / Wed 22: maddy=W3, james=W22, gen=W37, pv=W24, rory=W8, franc=W21, aysar=W21, john_yi=W20, rebecca=W21
"""
import time, json
    ...
    for row in rows:
        ...
        if not in_day:
            ...
            if cell_a and any(cell_a.startswith(d) for d in DAY_MARK):
                break
            # Skip first empty row of day
            if first_in_day and not any((c.strip() if c else "") for c in row):
                first_in_day = False
                continue
            first_in_day = False
...
print(f"=== Task log check — Thu 23/04/2026 ===\n")
...
print(f"{'Dev':10s} | {'Thu(h)':7s} | {'Leave':12s} | {'Alert':5s} | Breakdown")
for dev, d in sorted(devs.items()):
    ...
    if leave == 'full_day_off':
        ...
        alert = True
        st = 'ZERO (ALERT)'
    print(f"{dev:10s} | {h:7.2f} | {leave or '-':12s} | {'YES' if alert else 'no':5s} | {' '.join(d['sheets'])}")

with open('/tmp/tasklog_apr23.json', 'w') as f:
    json.dump({'per_sheet': results, 'per_dev': devs}, f, indent=2, default=str)
print("\nJSON saved to /tmp/tasklog_apr23.json")
