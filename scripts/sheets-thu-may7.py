#!/usr/bin/env python3
"""Cross-project task-log scan for Thu 2026-05-07.

Reads ALL configured task-log sheets, finds the current week tab via Summary,
then sums per-employee hours on Thu 07/05/26 (col A = 'Task dự án'). Aggregates
across projects per owner.

Special cases:
- Aysar uses NON-CALENDAR W{n} naming. W23 = May 4-10 2026. Detect via tab first
  date row.
- Rebecca: LeNH lives in cols Q-T (idx 16-19) instead of G/H.
- Paturevision: skip first empty row of each day (write reserved).
- LongVV W25 PARTIAL schedule — sub-8h is OK, no alert (rule expires 2026-05-10).
"""
import json
import time
...
def find_week_tab_by_first_date(sheet_id, target_token="04/05/26"):
    """Aysar fallback: scan W{n} tabs, find one whose first day-header contains target_token."""
    tabs = list_tab_names(sheet_id)
    candidates = [t for t in tabs if t.startswith("W")]
    # Prefer higher numbers first (recent weeks)
    def num(t):
        try:
            return int(t[1:])
        except Exception:
...
def scan_week_tab_owner_g(sheet_id, week, project_label=""):
    """Return (rows_out, leave_markers, raw_rows) for Thu block.

    Paturevision rule: skip first empty Task dự án row of the day (write reserved).
    """
    ...
    for i in range(thu_start, next_idx):
        ...
        if str(a) != "Task dự án":
            continue
        owner = str(g).strip()
        # Paturevision skip first empty (write reserved) row
...
def main():
    ...
    for project, sid in SHEETS.items():
        ...
        if project == "Aysar":
            ...
            if not wk:
                # try Sat 02/05/26 (some sheets start Sat)
                wk = find_week_tab_by_first_date(sid, target_token="02/05/26")
            if not wk:
                wk = find_week_tab_by_first_date(sid, target_token="07/05/26")
        else:
    ...
    print("\n\n=== AGGREGATE per dev (Thu 07/05/26) ===")
    ...
    with open("/tmp/sheets_thu_may7.json", "w") as fh:
        json.dump(out, fh, indent=2, default=str)
    print("\nWrote /tmp/sheets_thu_may7.json")
    return out
