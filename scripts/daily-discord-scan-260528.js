// Daily Discord scan for 2026-05-28 — window: 2026-05-27T08:56+07 → now
...
const WINDOW_EPOCH_MS = new Date("2026-05-27T08:56:29+07:00").getTime();
...
async function verifyToken(token) {
  ...
  if (r.status !== 200) return { valid: false, reason: `users/@me status ${r.status}` };
  const g = await discordRequest("/users/@me/guilds", token);
  return { valid: g.status === 200, reason: g.status !== 200 ? `guilds status ${g.status}` : "ok" };
}
...
async function scanAccount(acct) {
  ...
  if (!verify.valid) return { account: acct.name, error: "token_invalid: " + verify.reason, messages: [] };
  ...
  return { account: acct.name, tokenValid: true, messages: allMessages };
}

// Also check Andrew Taraba DM from nuscarrick
...
(async () => {
  ...
  for (const acct of accounts) {
    if (acct.name && acct.name.toLowerCase().includes("carrick")) {
      // Bizurk + Andrew DM check
      const scan = await scanAccount(acct);
      const dms = await checkAndyDM(acct);
      output[acct.name] = { ...scan, andrewDMs: dms };
    } else {
      output[acct.name] = await scanAccount(acct);
    }
  }
