#!/usr/bin/env python3
"""Piece 2 scan for 2026-04-23 daily report.
Window: 2026-04-21 08:27 +07 -> 2026-04-23 08:55 +07.
"""
import json
import os
import time
...
CUTOFF_START = 1776734820  # 2026-04-21 08:27:00 +07:00
CUTOFF_END = 1776909300    # 2026-04-23 08:55:00 +07:00
QUERY = "after:2026-04-20"  # slack `after:` excludes named date, so use day before start
...
def summarize(result):
    ...
    for ch_name, ch_msgs in sorted(channels.items(), key=lambda x: -len(x[1])):
        ...
        if len(ch_msgs) > 6:
            lines.append(f"  - ... and {len(ch_msgs)-6} more")
    return "\n".join(lines) + "\n"


if __name__ == "__main__":
    print(f"# Slack Piece 2 Scan — {time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    print(f"Window: 2026-04-21 08:27 -> 2026-04-23 08:55 +07 (epoch {CUTOFF_START}..{CUTOFF_END})")
    print(f"Query: {QUERY}")
    print()
