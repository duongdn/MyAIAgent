...
function decodeSubject(str) {
  return str.replace(/=\?[^?]+\?[BQ]\?([^?]*)\?=/gi, (m, enc) => {
    try { return Buffer.from(enc.replace(/_/g,' '), 'base64').toString('utf8'); } catch(_) { return enc; }
  });
}
    ...
    socket.on("data", (data) => {
      ...
      if (step === 0 && buffer.includes("OK")) {
        ...
        for (const blk of blocks) {
          ...
          if (!subM) continue;
          const subj = decodeSubject(subM[1].trim());
        ...
        resolve({ email: acct.email, count: subjects.length, subjects: subjects.slice(0, 15), alerts });
      }
    });
  });
}
