#!/usr/bin/env node
/**
 * Daily sheets scan for 2026-06-03 (Wednesday) — W29 (June 1-7, 2026).
 * VuTQ returned to Fountain from W29. LongVV: part-time 16h/wk.
 */
...
const SHEETS = {
  ...
  TuanNT_Neural:"1drk_TN7-B2xD43jgErH5aWGaeCsIMtNbiIUTNbFYheg",  // TuanNT 3rd project (Neural)
...
const WED_TOKENS = ["Wed, 03/06/26", "03/06/26", "Wed, 3/06/26"];
const TARGET_DATE = new Date(2026, 5, 3); // June 3, 2026
...
async function getWeeklySummary(api, id, tab) {
  try {
    for (const row of await fetchRange(api, id, "Summary!A1:E100"))
      if (row && String(row[0]).trim() === tab) return parseHours(row[3] || "");
  } catch (e) {}
  return null;
}
...
async function main() {
  ...
  results.PhucVT = { todayHours: sum(jdH), weeklyTotal: tabs.JamesDiamond ? await getWeeklySummary(api, SHEETS.JamesDiamond, tabs.JamesDiamond) : null, leave: jdL, err: jdE, owners: jdH };
  ...
  results.VietPH = {
    ...
    weeklyTotal: tabs.Paturevision ? await getWeeklySummary(api, SHEETS.Paturevision, tabs.Paturevision) : null,
  ...
  results.KhanhHH = {
    generatorHours: Object.keys(khGen).length ? sum(khGen) : sum(genH),
    elenaHours: sum(khElena),
    totalHours: sum(khGen) + sum(khElena) || sum(genH),
    weeklyTotal: tabs.Generator ? await getWeeklySummary(api, SHEETS.Generator, tabs.Generator) : null,
    leave: genL, err: genE, generatorOwners: genH, elenaOwners: elenaAll,
  };
  ...
  results.LongVV = {
    ...
    weeklyTotal: tabs.Maddy ? await getWeeklySummary(api, SHEETS.Maddy, tabs.Maddy) : null,
