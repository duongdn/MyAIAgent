#!/usr/bin/env node
/**
 * Daily sheets scan for 2026-06-23 (Tuesday) — checking June 22 (PREV_DATE, Monday) hours.
 * KhanhHH: ALL sheets + Workstream Baamboozle + Colin/ETZ (mandatory).
 * LeNH: Rory + Franc + Rebecca (filter col G=LeNH, col H — NOT Q-T per 2026-06-22 correction).
 */
...
// Jun 22 (PREV_DATE, Monday) — what we scan for hours
const PREV_TOKENS = ["Mon, 22/06/26", "22/06/26"];
const TARGET_DATE = new Date(2026, 5, 22); // June 22, 2026
const PREV_DATE   = new Date(2026, 5, 22);
const WS_DATE     = "2026-06-22";

// Workstream project IDs for KhanhHH
const WS_KHANHHH_BAAMBOOZLE = "cmqez93ka07q8p81v7035l3td";
const WS_KHANHHH_COLIN_ETZ  = "cmqezatb807qvp81vpnzzimmp";

// Blair Brown WP project (KhanhHH may work there too)
const WS_BLAIR_BROWN = "cmqj4tj6v01gfm81vgx7ipkov";
...
async function discoverWeekTab(api, id, targetDate, fallback) {
  try {
    ...
    process.stderr.write(`  discovery err: ${e.message}\n`);
  }
  if (fallback) {
    try {
...
async function getOwnerWeeklyTotal(api, id, tab, ownerSubstr) {
  ...
  try {
    ...
    process.stderr.write(`  ownerWeekly err (${ownerSubstr}): ${e.message}\n`);
  }
  return null;
}

async function fetchWorkstreamHours(projectId, date, employeeSubstr) {
  try {
    ...
    if (!res.ok) return { hours: 0, err: "HTTP " + res.status, rows: [] };
    ...
    for (const row of rows) {
      ...
      if (employeeSubstr && !name.toLowerCase().includes(employeeSubstr.toLowerCase())) continue;
      const h = parseHoursHM(row.actual || row.actualHours || "0:00");
      total += h;
      if (h > 0) matched.push({ name, hours: h });
    }
    return { hours: total, err: null, rows: matched };
  } catch (e) {
    return { hours: 0, err: e.message, rows: [] };
  }
}
...
async function main() {
  ...
  process.stderr.write("Discovering week tabs for Jun 22...\n");
  const tabs = {};
  for (const [name, id] of Object.entries(SHEETS)) {
    process.stderr.write(`  ${name}...\n`);
    tabs[name] = await discoverWeekTab(api, id, TARGET_DATE, null);
    process.stderr.write(`  ${name} → ${tabs[name]}\n`);
  }

  // TuanNT — 5 sheets (JohnYi, Rebecca, Paturevision, TuanNT_Neural, CharlesChang)
  process.stderr.write("TuanNT (5 sheets)...\n");
  ...
  results.TuanNT = {
    johnyiHours: sum(jyH), rebeccaHours: sum(rbH),
    paturevisionHours: sum(patTuant), neuralHours: sum(neuralTuant), charlesChangHours: sum(ccTuant),
    ...
    neuralOwners: neuralTuant, ccOwners: ccTuant,
  };
  ...
  results.VietPH = { todayHours: sum(vietphH), leave: patL, err: patE, allOwners: patAll };

  // KhanhHH — scan ALL sheets + Workstream (3 WS sources)
  process.stderr.write("KhanhHH (ALL sheets + Workstream)...\n");
  const khAllSheets = {};
  const khAllLeave = {};
  for (const [sname, sid] of Object.entries(SHEETS)) {
    ...
    Object.entries(leaveNotes)
      .filter(([k]) => k.includes("KhanhHH") || k === "ALL")
      .forEach(([k, v]) => { khAllLeave[`${sname}:${k}`] = v; });
  }
  const khSheetsTotal = Object.values(khAllSheets).reduce((a, b) => a + b, 0);

  process.stderr.write("KhanhHH (Workstream Baamboozle)...\n");
  const wsBaam = await fetchWorkstreamHours(WS_KHANHHH_BAAMBOOZLE, WS_DATE, "KhanhHH");
  process.stderr.write(`  Baamboozle: ${wsBaam.hours}h (${wsBaam.err || "ok"})\n`);

  process.stderr.write("KhanhHH (Workstream Colin/ETZ)...\n");
  const wsEtz = await fetchWorkstreamHours(WS_KHANHHH_COLIN_ETZ, WS_DATE, "KhanhHH");
  process.stderr.write(`  Colin/ETZ: ${wsEtz.hours}h (${wsEtz.err || "ok"})\n`);

  process.stderr.write("KhanhHH (Workstream Blair Brown)...\n");
  const wsBlair = await fetchWorkstreamHours(WS_BLAIR_BROWN, WS_DATE, "KhanhHH");
  process.stderr.write(`  Blair: ${wsBlair.hours}h (${wsBlair.err || "ok"})\n`);

  results.KhanhHH = {
    sheetsTotal: khSheetsTotal,
    baamboozleWorkstream: wsBaam.hours,
    colinEtzWorkstream: wsEtz.hours,
    blairWorkstream: wsBlair.hours,
    totalHours: khSheetsTotal + wsBaam.hours + wsEtz.hours + wsBlair.hours,
    bySheet: khAllSheets,
    leave: khAllLeave,
    wsErrors: { baamboozle: wsBaam.err, colinEtz: wsEtz.err, blair: wsBlair.err },
  };

  // Elena sheet hours (separate from KhanhHH)
  let elenaAll = {}, elenaL = {}, elenaE = null;
  if (tabs.Elena) ({ ownerHours: elenaAll, leaveNotes: elenaL, err: elenaE } = extractDailyHoursByOwner(
    await fetchRange(api, SHEETS.Elena, `${tabs.Elena}!A:I`), tokens
  ));
  results.Elena = { tab: tabs.Elena, todayHours: sum(elenaAll), owners: elenaAll, leave: elenaL, err: elenaE };

  // LeNH — Rory + Franc + Rebecca (filter col G=LeNH, col H — corrected 2026-06-22, NOT Q-T)
  process.stderr.write("LeNH (Rory + Franc + Rebecca by owner)...\n");
  let roryH = {}, roryL = {}, roryE = null;
  if (tabs.Rory) ({ ownerHours: roryH, leaveNotes: roryL, err: roryE } = extractDailyHoursByOwner(
    await fetchRange(api, SHEETS.Rory, `${tabs.Rory}!A:I`), tokens
  ));
  let francH = {}, francL = {}, francE = null;
  if (tabs.Franc) ({ ownerHours: francH, leaveNotes: francL, err: francE } = extractDailyHoursByOwner(
    await fetchRange(api, SHEETS.Franc, `${tabs.Franc}!A:I`), tokens
  ));
  // Rebecca: filter col G=LeNH, col H (NOT Q-T)
  const rbLeNHFromOwner = Object.fromEntries(
    Object.entries(rbH).filter(([k]) => k.toLowerCase().includes("lenh"))
  );
  ...
  results.LeNH = {
    roryHours: sum(roryLeNH),
    francHours: sum(francLeNH),
    rebeccaLeNHHours: sum(rbLeNHFromOwner),
    totalHours: sum(roryLeNH) + sum(francLeNH) + sum(rbLeNHFromOwner),
    leave: { ...roryL, ...francL, ...rbL },
    errors: { rory: roryE, franc: francE, rebecca: rbE },
    roryOwners: roryH,
    francOwners: francH,
    rebeccaLeNHOwners: rbLeNHFromOwner,
  };

  // LongVV (Maddy sheet, part-time 16h/wk)
  process.stderr.write("LongVV (Maddy)...\n");
  let longvvH = {}, maddyL = {}, maddyE = null;
  if (tabs.Maddy) ({ ownerHours: longvvH, leaveNotes: maddyL, err: maddyE } = extractDailyHoursByOwner(
    await fetchRange(api, SHEETS.Maddy, `${tabs.Maddy}!A:I`), tokens
  ));
