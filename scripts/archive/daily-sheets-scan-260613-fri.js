#!/usr/bin/env node
/**
 * Daily sheets scan for 2026-06-13 (Saturday) — checking June 12 (PREV_DATE, Friday) hours.
 * W44 (June 8-14, 2026).
 */
...
// Jun 12 (PREV_DATE, Friday) — what we scan for hours today
const PREV_TOKENS = ["Fri, 12/06/26", "12/06/26"];
const TARGET_DATE = new Date(2026, 5, 13); // June 13, 2026 — used for week tab discovery
const PREV_DATE   = new Date(2026, 5, 12); // June 12, 2026 — daily hours to report
...
async function main() {
  ...
  const tokens = PREV_TOKENS; // Jun 12 (Friday) data
  const results = {};
  ...
  // TuanNT — ON LEAVE Jun 12 (approved). Still scan to confirm 0h and note leave.
  ...
  results.TuanNT = {
    ...
    onLeave: true,
    note: "TuanNT on approved leave Jun 12. 0h expected.",
  };
  ...
  // LeNH — ON LEAVE Jun 12. Still scan to confirm 0h.
  ...
  results.LeNH = {
    ...
    onLeave: true,
    note: "LeNH on approved leave Jun 12. 0h expected. Aysar sheet owner=KhanhHH — excluded from LeNH",
  };
