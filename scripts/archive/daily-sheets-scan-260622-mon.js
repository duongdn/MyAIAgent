#!/usr/bin/env node
/**
 * Daily sheets scan for 2026-06-22 (Monday) — checking June 20 (PREV_DATE, Friday) hours.
 * W28 (June 15-21, 2026) — June 20 falls at end of W28.
 * KhanhHH: Generator sheet + Baamboozle Workstream + Colin/ETZ Workstream (all 3 mandatory).
 */
...
// Jun 20 (PREV_DATE, Friday) — what we scan for hours
const PREV_TOKENS = ["Fri, 20/06/26", "20/06/26"];
const TARGET_DATE = new Date(2026, 5, 20); // June 20, 2026 — used for week tab discovery
const PREV_DATE   = new Date(2026, 5, 20); // June 20, 2026 — daily hours to report

// Workstream project IDs for KhanhHH (3 sources mandatory)
const WS_KHANHHH_BAAMBOOZLE = "cmqez93ka07q8p81v7035l3td";
const WS_KHANHHH_COLIN_ETZ  = "cmqezatb807qvp81vpnzzimmp";
const WS_DATE = "2026-06-20"; // Friday
...
// Workstream API: fetch week data for a project, filter by employeeName containing substr
...
async function main() {
  ...
  // PhucVT (JamesDiamond sheet) — filter by owner only
  ...
  // KhanhHH — scan ALL sheets + Workstream Baamboozle + Workstream Colin/ETZ (ALL 3 sources)
  process.stderr.write("KhanhHH (sheets)...\n");
  ...
  // KhanhHH Workstream — Baamboozle project
  ...
  // KhanhHH Workstream — Colin/ETZ project
  ...
  results.KhanhHH = {
    ...
    totalHours: khSheetsTotal + wsBaam.hours + wsEtz.hours,
    bySheet: khAllSheets,
    weeklyTotal: (khGenWeekly || 0) + (khAysarWeekly || 0),
    leave: khAllLeave,
    wsErrors: { baamboozle: wsBaam.err, colinEtz: wsEtz.err },
