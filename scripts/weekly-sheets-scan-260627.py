#!/usr/bin/env python3
"""Weekly sheets scan for W32 (June 22-27, 2026). Run: 2026-06-27 (Friday).
...
SHEETS = {
    ...
    "CharlesChang":  "1RGBUq6iJP0VGlGDAX_9B4lMJFpNAY3e2J0MOXJ_Lyeo",
}

# W32 target: June 22 (Mon) - June 27 (Fri) 2026
TARGET_MONDAY = date(2026, 6, 22)
TARGET_FRIDAY = date(2026, 6, 27)
...
# ── Discover tabs ──────────────────────────────────────────────────────────────
print("\n=== Discovering W32 tabs (target: 2026-06-22) ===", file=sys.stderr)
tabs = {}
...
results = {"tabs": tabs, "week": "W32 (Jun 22-27, 2026)"}
...
print("\n=== Paturevision (VietPH, VuTQ, TuanNT, DuongDN) ===", file=sys.stderr)
...
duongdn_pat = sum(v for k, v in pat_owners.items() if "DuongDN" in k or k == "DuongDN")

results["VietPH"] = {"total": vietph_total, "target": 40, "leave": pat_leave,
                      "note": "LAST WEEK (final day was Jun 26 Thu)"}
results["VuTQ_pat"] = {"total": vutq_pat_total}
results["DuongDN_pat"] = {"total": duongdn_pat}

# ── TuanNT: JohnYi + Rebecca + Paturevision + Neural + CharlesChang ──────────
print("\n=== TuanNT ===", file=sys.stderr)
jy_tab = tabs.get("JohnYi")
rb_tab = tabs.get("Rebecca")
neural_tab = tabs.get("Neural")
cc_tab = tabs.get("CharlesChang")

tuannt_jy = 0.0; tuannt_rb = 0.0; tuannt_neural = 0.0; tuannt_cc = 0.0
...
if cc_tab:
    cc_owners, cc_leave = get_wtab_owner_totals(SHEETS["CharlesChang"], cc_tab)
    tuannt_cc = sum(v for k, v in cc_owners.items() if "TuanNT" in k or k == "TuanNT")
    if tuannt_cc == 0:
        tuannt_cc = sum(cc_owners.values())
    print(f"  CharlesChang owners: {cc_owners}", file=sys.stderr)

tuannt_total = tuannt_jy + tuannt_rb + tuannt_pat + tuannt_neural + tuannt_cc
all_leave = {**jy_leave, **rb_leave, **neural_leave, **pat_leave, **cc_leave}
...
# ── KhanhHH: Generator + Elena + Aysar ───────────────────────────────────────
print("\n=== KhanhHH ===", file=sys.stderr)
gen_tab = tabs.get("Generator")
elena_tab = tabs.get("Elena")
aysar_tab = tabs.get("Aysar")

khanhhh_gen = 0.0; khanhhh_elena = 0.0; khanhhh_aysar = 0.0
gen_owners, gen_leave, elena_owners, elena_leave, aysar_owners, aysar_leave = {}, {}, {}, {}, {}, {}
...
if aysar_tab:
    ...
    khanhhh_aysar_found = sum(v for k, v in aysar_owners.items() if "KhanhHH" in k or k == "KhanhHH")
    if khanhhh_aysar_found > 0:
        khanhhh_aysar = khanhhh_aysar_found
    else:
        khanhhh_aysar = sum(aysar_owners.values())
    print(f"  Aysar owners: {aysar_owners}", file=sys.stderr)

khanhhh_total = khanhhh_gen + khanhhh_elena + khanhhh_aysar
results["KhanhHH"] = {
    "generatorHours": khanhhh_gen,
    "elenaHours": khanhhh_elena,
    "aysarHours": khanhhh_aysar,
    "total": khanhhh_total,
    "target": 40,
    "leave": {**gen_leave, **elena_leave, **aysar_leave},
    "genOwners": gen_owners,
    "elenaOwners": elena_owners,
    "aysarOwners": aysar_owners,
}

# ── LeNH: Rory + Franc + Rebecca (Q-T) ────────────────────────────────────────
...
rory_owners, rory_leave, franc_owners, franc_leave2 = {}, {}, {}, {}
