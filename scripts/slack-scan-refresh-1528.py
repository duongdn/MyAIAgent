#!/usr/bin/env python3
"""Slack refresh scan for 2026-05-05 15:28 refresh.
Window: 2026-05-05 09:30 +07 -> now (~6h).
Only NEW messages since the morning daily report.
"""
import json
import os
import time
...
CUTOFF_EPOCH = 1777948200  # 2026-05-05 09:30:00 +07:00
QUERY = "after:2026-05-04"  # slack `after:` excludes named date
...
def summarize(result):
    ...
    for ch_name, ch_msgs in sorted(channels.items(), key=lambda x: -len(x[1])):
        ...
        if len(ch_msgs) > 10:
            lines.append(f"  - ... and {len(ch_msgs)-10} more")
    return "\n".join(lines) + "\n"


if __name__ == "__main__":
    print(f"# Slack Refresh Scan — {time.strftime('%Y-%m-%d %H:%M:%S %Z')}")
    print(f"Cutoff: 2026-05-05 09:30:00 +07 (epoch {CUTOFF_EPOCH})")
    print(f"Query: {QUERY}")
    print()
