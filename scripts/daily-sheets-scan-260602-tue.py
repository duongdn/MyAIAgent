#!/usr/bin/env python3
"""Daily sheets scan for 2026-06-02 (Tuesday).
Checks Tuesday 2026-06-02 hours — new week (June 2-8).

Note: LongVV on sick leave (per email). VuTQ in Paturevision (Bailey).

Week tabs are discovered dynamically from Summary tabs.
Fallback guesses based on prev week (+1):
  Maddy:        W9  (June 1-7)
  JohnYi:       W26 (June 1-7)
  Rebecca:      W27 (June 1-7)
  JamesDiamond: W28 (June 1-7)  -- PhucVT sheet
  Rory:         W14 (June 1-7)
  Franc:        W27 (June 1-7)
  Aysar:        W27 (June 1-7)  -- non-calendar, need discovery
  Generator:    W43 (June 1-7)  -- KhanhHH sheet
  Paturevision: W30 (June 1-7)  -- VietPH + VuTQ (Bailey DEV1)
  Elena:        discover from Summary
"""
import json
import re
import sys
...
# Fallback week tabs (guessed from prev week +1)
FALLBACK_TABS = {
    "Maddy":         "W9",
    "JohnYi":        "W26",
    "Rebecca":       "W27",
...
TUE_JUN02_TOKENS = ["Tue, 02/06/26", "02/06/26"]
...
def discover_week_tab(sheet_id, target_dt, fallback=None):
    """Find week tab from Summary!A:E by matching target_dt to date range."""
    ...
    for row in rows:
        ...
        if start_d and end_d and start_d <= target_dt <= end_d:
            print(f"  Discovered tab {tab} for {target_dt} ({start_d}→{end_d})", file=sys.stderr)
            return tab
    # Fallback: try guessing
    if fallback:
        test = fetch(sheet_id, f"{fallback}!A1:A3")
        if test and "ERROR" not in str(test[0]):
            print(f"  Using fallback tab {fallback}", file=sys.stderr)
            return fallback
    return None
...
print(f"\nWeek tabs: {json.dumps(curr_tabs)}", file=sys.stderr)
...
# --- KhanhHH: Generator + Elena sheets (flex) ---
print("Checking KhanhHH (Generator+Elena)...", file=sys.stderr)
...
if khanhhh_total == 0:
    # If no KhanhHH-filtered rows, use all rows from Generator as fallback
...
results["KhanhHH"] = {
    ...
    "generatorOwners": khanhhh_hours,
    "elenaOwners": elena_hours_all,
    "leave": gen_leave,
    "err": gen_err,
}
...
# Note: LongVV on sick leave today per email — 0h expected
...
results["LongVV"] = {
    ...
    "leave": maddy_leave,
    "err": maddy_err,
    "note": "Part-time 16h/wk — sick leave today per email. 0h expected.",
}

# --- Elena sheet summary (already fetched above) ---
results["Elena"] = {
    "tab": curr_tabs.get("Elena"),
    "hours": sum(elena_hours_all.values()),
    "owners": elena_hours_all,
    "leave": elena_leave,
    "err": elena_err,
}

# --- All sheet tabs summary ---
