// Daily Discord scan for 2026-05-29 — window: 2026-05-28T08:55+07 → now
...
const WINDOW_EPOCH_MS = new Date("2026-05-28T08:55:00+07:00").getTime();
...
async function scanAccount(acct) {
  ...
  if (!verify.valid) return { account: acctName, error: "token_invalid: " + verify.reason, messages: [] };

  const guildsRes = await discordRequest("/users/@me/guilds", token);
  if (guildsRes.status !== 200) return { account: acctName, error: "guilds_fetch_failed", messages: [] };
