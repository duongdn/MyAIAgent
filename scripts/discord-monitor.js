// Reusable Discord monitor — reads last_run from .monitoring-timelines.json dynamically.
// Usage: node scripts/discord-monitor.js [--since=ISO8601]
// Only scans AirAgri (nusvinn) + Bizurk (nuscarrick). NOT HOMIEAPP.
// Output: JSON { account: { tokenValid, messages: [...], andrewDMs?: [...] } }
const https = require("https");
const fs = require("fs");
const path = require("path");

const ROOT = path.resolve(__dirname, "..");
const cfg = JSON.parse(fs.readFileSync(path.join(ROOT, "config/.discord-accounts.json")));
const timelines = JSON.parse(fs.readFileSync(path.join(ROOT, "config/.monitoring-timelines.json")));
const accounts = cfg.accounts || cfg;

// --since=ISO8601 arg overrides timeline last_run
const sinceArg = process.argv.find(a => a.startsWith("--since="));
const sinceStr = sinceArg
  ? sinceArg.split("=")[1]
  : timelines.daily_report?.last_run;

if (!sinceStr) {
  console.error("No since time: set daily_report.last_run in .monitoring-timelines.json or pass --since=ISO8601");
  process.exit(1);
}

const WINDOW_EPOCH_MS = new Date(sinceStr).getTime();
const DISCORD_EPOCH = 1420070400000n;
const WINDOW_SNOWFLAKE = String((BigInt(Math.floor(WINDOW_EPOCH_MS)) - DISCORD_EPOCH) << 22n);

process.stderr.write(`Discord monitor: window from ${sinceStr} (snowflake ${WINDOW_SNOWFLAKE})\n`);
...
async function scanAccount(acct) {
  ...
  if (guildsRes.status !== 200) {
    return { account: acctName, tokenValid: false, error: "guilds_fetch_failed status " + guildsRes.status, messages: [] };
  }
